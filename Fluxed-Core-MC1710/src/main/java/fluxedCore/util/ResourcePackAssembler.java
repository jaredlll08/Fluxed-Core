...
public class ResourcePackAssembler
{
	...
	public ResourcePackAssembler assemble()
	{
		...
		try
		{
			...
			if (hasPackPng)
			{
				fluxedCore.util.FileUtils.copyFromJar(jarClass, modid + "/" + "pack.png", new File(dir.getAbsolutePath() + "/pack.png"));
			}
			...
			fluxedCore.util.FileUtils.zipFolderContents(dir, zip);
			dir.delete();
		} catch (IOException e)
		{
